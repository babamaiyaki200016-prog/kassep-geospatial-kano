# ============================================================================
# KASSEP Geospatial Analysis – COMBINED retrospective + prospective (n=116)
# ============================================================================
# Title: Geospatial determinants of maternal mortality in Kano State, Nigeria
# Forked from run_analysis.R (n=93 retrospective only) to add the 23
# prospective-wave deaths (prospective_va.xlsx, GPS via
# "_Please pick the location_latitude/longitude"). All outputs write to
# outputs_n116/ so the original n=93 outputs/manuscript numbers are untouched.
# Date: 2026-07-16
...
PROSPECTIVE_EXCEL <- "/Users/babamusa/Downloads/prospective_va.xlsx"
...
dir.create("outputs_n116", showWarnings = FALSE)

# ----------------------------------------------------------------------------
# 1. CHECK INPUT FILES
# ----------------------------------------------------------------------------

files_to_check <- c(KASSEP_EXCEL, PROSPECTIVE_EXCEL, STATE_SHP, LGA_SHP, DEPRIV_CSV, POP_RASTER)
...
# 3.1 KASSEP -- combine retrospective (n=93) + prospective (n=23) waves.
# Only Lat/Lng/lganame and the place-of-death item are used downstream, but
# bind_rows() keeps every column from both raw VA exports (NA where a
# column exists in only one wave), so nothing else is silently dropped.
X03_retro <- read_excel(KASSEP_EXCEL)
X03_pros  <- read_excel(PROSPECTIVE_EXCEL) %>%
  rename(Lat = `_Please pick the location_latitude`,
         Lng = `_Please pick the location_longitude`)
X03 <- bind_rows(
  X03_retro %>% mutate(wave = "retrospective"),
  X03_pros  %>% mutate(wave = "prospective")
)
cat("✅ Combined dataset: ", nrow(X03_retro), " retrospective + ", nrow(X03_pros),
    " prospective = ", nrow(X03), " total.\n", sep = "")
...
# ----------------------------------------------------------------------------
# 4. CLEAN GPS – INCLUDE ALL 116 DEATHS (93 retrospective + 23 prospective)
# ----------------------------------------------------------------------------
...
cat("✅ GPS cleaned: ", nrow(df_clean), " deaths (target 116).\n")
...
write.csv(moran_results, "outputs_n116/moran_comparison.csv", row.names = FALSE)
...
write.csv(hotspot_summary, "outputs_n116/table2_hotspot_lgas.csv", row.names = FALSE)

# ---- 8b. HOTSPOT DETECTION ON 10 KM GRID (comparison, not in original script --
#      added because 5km clustering is no longer significant with n=116; see
#      whether the now-significant 10km signal changes the hotspot picture) ----
coords_10km_hot <- st_centroid(grid_10km_filtered) %>% st_coordinates()
nb_knn5_10km <- knn2nb(knearneigh(coords_10km_hot, k = 5))
listw_knn5_10km <- nb2listw(nb_knn5_10km, style = "W")
gi_10km <- localG(grid_10km_filtered$deaths, listw_knn5_10km, zero.policy = TRUE)
grid_10km_filtered$gi_z <- as.numeric(gi_10km)
grid_10km_filtered <- grid_10km_filtered %>%
  mutate(hotspot = case_when(
    gi_z >= 2.58 ~ "Very Hot",
    gi_z >= 1.96 ~ "Hot",
    gi_z >= 1.65 ~ "Warm",
...
hotspot_cells_10km <- grid_10km_filtered %>% filter(hotspot %in% c("Very Hot", "Hot"))
hotspot_lgas_10km <- st_join(hotspot_cells_10km, kano_lgas_utm, largest = TRUE)
lga_col_hot_10km <- grep("lganame", names(hotspot_lgas_10km), value = TRUE, ignore.case = TRUE)[1]
hotspot_lgas_10km$lganame <- hotspot_lgas_10km[[lga_col_hot_10km]]
hotspot_summary_10km <- hotspot_lgas_10km %>%
  st_drop_geometry() %>%
  group_by(lganame) %>%
  summarise(Hotspot_Cells = n()) %>%
  arrange(desc(Hotspot_Cells))
write.csv(hotspot_summary_10km, "outputs_n116/table2b_hotspot_lgas_10km.csv", row.names = FALSE)
cat("✅ 10km hotspot comparison complete: ", nrow(hotspot_cells_10km), " hotspot cells across ",
    nrow(hotspot_summary_10km), " LGAs (vs 5km: ", nrow(hotspot_cells), " cells / ",
    nrow(hotspot_summary), " LGAs).\n", sep = "")

p1b <- ggplot() +
  geom_sf(data = study_area, fill = "gray95", color = "black") +
  geom_sf(data = grid_10km_filtered, aes(fill = gi_z), color = NA) +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0, name = "Gi* z-score") +
  labs(title = "Getis-Ord Gi* Hotspots (10 km grid)") +
  theme_void()
ggsave("outputs_n116/fig1b_getis_hotspots_10km.png", p1b, width = 8, height = 7, dpi = 300)
...
write.csv(as.data.frame.matrix(cross_tab), "outputs_n116/table5_deprivation_hotspots.csv")
...
write.csv(res1, "outputs_n116/table3_regression_results.csv", row.names = FALSE)

fit1 <- glance(ols1)
fit_df <- data.frame(model = "Original", R2 = fit1$r.squared, Adj_R2 = fit1$adj.r.squared, p_F = fit1$p.value)
write.csv(fit_df, "outputs_n116/model_fit_comparison.csv", row.names = FALSE)
...
), "outputs_n116/residual_moran.csv", row.names = FALSE)
...
write.csv(data.frame(predictor = names(vif_enh), vif = vif_enh), "outputs_n116/table3b_vif_model2.csv", row.names = FALSE)
...
write.csv(all_models_tidy, "outputs_n116/table3_regression_results.csv", row.names = FALSE)
...
write.csv(all_models_fit, "outputs_n116/model_fit_comparison.csv", row.names = FALSE)
...
write.csv(all_models_residual_moran, "outputs_n116/residual_moran.csv", row.names = FALSE)
...
write.csv(priority_lgas, "outputs_n116/table4_priority_areas.csv", row.names = FALSE)
...
p1 <- ggplot() +
  ...
  theme_void()
ggsave("outputs_n116/fig1_getis_hotspots.png", p1, width = 8, height = 7, dpi = 300)
...
ggsave("outputs_n116/fig2_composite.png", p2, width = 10, height = 8, dpi = 300)
...
p3 <- ggplot() +
  ...
  theme_void()
ggsave("outputs_n116/fig3_predicted_risk.png", p3, width = 8, height = 7, dpi = 300)
...
p4 <- ggplot() +
  ...
  theme_void()
ggsave("outputs_n116/fig4_priority_areas.png", p4, width = 8, height = 7, dpi = 300)
...
write.csv(table1, "outputs_n116/table1_descriptive.csv", row.names = FALSE)
...
write.csv(dist_lga, "outputs_n116/table6_distance_by_lga.csv", row.names = FALSE)
...
write.csv(table7, "outputs_n116/table7_data_sources.csv", row.names = FALSE)
...
sink("outputs_n116/session_info.txt")
sessionInfo()
sink()
